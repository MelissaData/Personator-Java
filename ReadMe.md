...
# Personator API JAVA REST Sample Code

This is a sample code for Melissa's Personator API.
It is written in JAVA and accesses the API through REST.
This sample is intended to show the functionality of the Personator Web Service and how to call the service in JAVA.
...
Inside the folder will be the build under `personator` as well as an all-in-one `personator.zip`.
...
To run the example, navigate to `./target/personator/bin/` and run `personator.bat`.
